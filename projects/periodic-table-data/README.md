...
## Named colormap（前端色标 LUT）

电负性等连续色标在**开发期**用 matplotlib 采样，硬编码进前端 TS；浏览器运行时只查表，**不**把 colormap 库打进 HTML。

```powershell
cd d:\WorkSpace\Blog\ChemistryNotebook\projects\periodic-table-data
uv sync --extra colormap
uv run python -m periodic_table_data.export_colormap --name YlOrRd --n 256
uv run python -m periodic_table_data.export_colormap --name YlGnBu --n 256
uv run python -m periodic_table_data.export_colormap --name BuPu --n 256
```

前端当前用法：电负性 → [`YlOrRd.ts`](../periodic-table/src/data/colormaps/YlOrRd.ts)；原子半径 → [`YlGnBu.ts`](../periodic-table/src/data/colormaps/YlGnBu.ts)；共价半径 → [`BuPu.ts`](../periodic-table/src/data/colormaps/BuPu.ts)。换色标时改 `--name` 重跑，并更新前端 `colorScale.ts` 的 import。
